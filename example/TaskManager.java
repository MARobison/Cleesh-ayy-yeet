...
public class TaskManager implements ShellDependent{
    ...
    @Command(description="Varargs example")
    public void tag(@Param(name="task_id", description="assign tags to tasks") int task_id, @Param(name="tags", description="assign multiple tags")String ...tags ) {
        String stmt = "";
        String tag_IdStmt = "";
        for (String i : tags) {
    ...
    @Command(description="Varargs example")
    public void due(@Param(name="task_id", description="assign due dates to tasks") int task_id, @Param(name="date", description="assign due dates to tasks")String date) {
...
//Check database for actual data!
    ...
    public String active(@Param(name="tag_id", description="show active tasks") String label) {
      ...
      String sqlReturn = database.runQuery(stmt);
      String stmt2 = "SELECT task.task_id FROM assign, task WHERE tag_id='" + sqlReturn +"' AND assign.task_id = task.task_id AND task_status = 'active'";
    ...
    @Command(description="Varargs example")
    public void rename(@Param(name="task_id", description="rename task") int task_id, @Param(name="label", description="rename")String ... labels) {
        String result = "";
        for (String i : labels) {
            result += i + " ";
        }
    ...
    public String completed(@Param(name="tag_id", description="show completed tasks") String label) {
      String stmt = "SELECT tag_id FROM tag WHERE tag_label='" + label + "'";
      String sqlReturn = database.runQuery(stmt);
      String stmt2 = "SELECT task.task_id FROM assign, task WHERE tag_id='" + sqlReturn +"' AND assign.task_id = task.task_id AND task_status = 'completed'";
      String sqlReturn2 = database.runQuery(stmt2);
      String sqlReturn3 = "";
      String stmt3 = "";
      for (String i : sqlReturn2.split("\n")) {          
    ...
    public static void main(String[] args) throws IOException, JSchException {
        ...
        ShellFactory.createConsoleShell("GetItDone", "EVERYTHING IS DUE NOTHING IS DONE\n" +
